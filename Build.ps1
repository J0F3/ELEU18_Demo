...
Properties {
    ...
    $RequiredModules = @(@{Name='xDnsServer';Version='1.7.0.0'}, @{Name='xNetworking';Version='2.9.0.0'}, @{Name='xWebAdministration';Version='1.19.0.0'}) 
}
...
Task InstallModules -Depends GenerateEnvironmentFiles {
    ...
    Save-Module -Name poshspec -Repository 'PSGallery' -Path $ModuleArtifactPath -Force
}
...
Task CompileConfigs -Depends UnitTests, ScriptAnalysis, GenerateEnvironmentFiles {
    # Compile Configurations...
    "Starting to compile configuration..."
    $configs = Get-ChildItem $ConfigPath -Filter *.ps1
    foreach($config in $configs)
    {
        "compile configuration $($config.Name)"
        . $config.FullName
       & "$($config.BaseName)" -ConfigurationData "$ConfigPath\DevEnv.psd1" -OutputPath "$MOFArtifactPath\DevEnv\"
    }
...
Task Clean {
    ...
    #Make sure output path exist for MOF and Module artifacts
