...
Properties {
    ...
    $TargetNodes = (Get-ChildItem "$MOFArtifactPath\DevEnv").Basename
}

Task Default -depends IntegrationTests,AcceptanceTests
...
Task DeployModules -Depends Clean {
    ...
    $Sessions = New-PSSession -ComputerName $TargetNodes -ErrorAction Stop
...
Task DeployConfigs -Depends DeployModules {
    ...
    Start-DscConfiguration -path "$MOFArtifactPath\DevEnv" -Wait -Verbose -Force
}
...
Task IntegrationTests -Depends DeployConfigs {
    ...
    $Session = New-PSSession -ComputerName "DNS01" -ErrorAction Stop

    #Create a folder to store test script on remote noded
    Invoke-Command -Session $Session -ScriptBlock { $null = new-item \Tests\ -ItemType Directory -Force }
    Copy-Item -Path "$TestsPath\Integration\DNSServer.tests.ps1" -Destination "c:\Tests" -ToSession $Session -verbose
    
    #Run pester on remote node and collect results
    $PesterResults = Invoke-Command -Session $Session -ScriptBlock { Invoke-Pester -Path c:\Tests -OutputFile "c:\Tests\IntegrationTest_DNS01.xml" -OutputFormat NUnitXml -PassThru } 
    
    #Get Results xml from remote node
    Copy-Item -path "c:\Tests\IntegrationTest_DNS01.xml" -Destination "$TestResultsPath" -FromSession $Session #-ErrorAction Continue
    ...
    "Testing Web Server..."
    #Run Integration tests on target node
    $Session = New-PSSession -ComputerName "Web01" -ErrorAction Stop

    #Create a folder to store test script on remote noded
    Invoke-Command -Session $Session -ScriptBlock { $null = new-item \Tests\ -ItemType Directory -Force }
    Copy-Item -Path "$TestsPath\Integration\WebServer.tests.ps1" -Destination "c:\Tests" -ToSession $Session -verbose
    
    #Run pester on remote node and collect results
    $PesterResults = Invoke-Command -Session $Session -ScriptBlock { Invoke-Pester -Path c:\Tests -OutputFile "c:\Tests\IntegrationTest_Web01.xml" -OutputFormat NUnitXml -PassThru } 
    
    #Get Results xml from remote node
    Copy-Item -path "c:\Tests\IntegrationTest_Web01.xml" -Destination "$TestResultsPath" -FromSession $Session #-ErrorAction Continue
    Invoke-Command -Session $Session -ScriptBlock {remove-Item "c:\Tests\" -Recurse} #-ErrorAction Continue
